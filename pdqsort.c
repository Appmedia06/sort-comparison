...
size_t pdqsort_loop(void *_begin,
                  void *_end,
                  size_t size,
                  cmp_func_t cmp_func,
                  size_t max_depth, 
                ...
                if (part_length > 0) {
                    ...
                    do {
                        ...
                        while (j <= part_length) {
                            if (j < part_length) {
                                comparison_count++;
                                j += (cmpint64(begin + idx(j), begin + idx(j + 1)) <0);
                            }
                            comparison_count++;
...
size_t pdqsort(void *base,
             size_t num,
             size_t size,
             cmp_func_t cmp_func,
             swap_func_t swap_func)
{
    return pdqsort_loop(base, base + idx(num), size, cmp_func, __log2(num), true);
}
